...
@section('title', 'Buat Purchase Order - Makmur Mandiri Medika')
@section('page-title', 'Buat Purchase Order')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-1">Buat Purchase Order Baru</h2>
        <p class="text-muted mb-0">Buat PO untuk pembelian produk dari supplier</p>
    </div>
...
<form method="POST" action="{{ route('admin.purchase-orders.store') }}" id="poForm" class="card p-4">
    @csrf
    ...
    <div id="itemsContainer">
        ...
        <div class="col-md-3">
            ...
            <input type="text" name="items[0][packaging]" class="form-control packaging-input" placeholder="Contoh: Box">
        </div>
        <div class="col-md-2">
        ...
        <div class="col-md-2 d-flex align-items-end">
            ...
            <div class="row mt-2">
                <div class="col-12">
                    ...
                    <input type="text" name="items[0][notes]" class="form-control item-notes" placeholder="Catatan untuk item ini">
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!itemsContainer || !addItemBtn) {
        console.error('PO Form: Elements not found:', { itemsContainer, addItemBtn });
        return;
    }
    
    let itemIndex = itemsContainer.querySelectorAll('.item-row').length;
    console.log('PO Form: Initial item count:', itemIndex);

    addItemBtn.addEventListener('click', () => {
        console.log('PO Form: Add button clicked!');
        const template = itemsContainer.querySelector('.item-row');
        if (!template) {
            console.error('PO Form: Template not found!');
            return;
        }
        ...
        itemIndex++;
        console.log('PO Form: New item added. Total items:', itemIndex);
    });
    ...
    itemsContainer.addEventListener('click', (event) => {
        if (event.target.closest('.remove-item')) {
            ...
            if (rows.length <= 1) {
                console.log('PO Form: Cannot remove last item');
                return;
            }
            event.target.closest('.item-row').remove();
            console.log('PO Form: Item removed. Remaining items:', rows.length - 1);
        }
    });
    
    console.log('PO form JavaScript initialized successfully');
});
</script>
@endpush
@endsection
